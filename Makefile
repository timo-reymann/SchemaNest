...
build-darwin: create-dist  ## Build binaries for macOS
	@CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN_PREFIX_SCHEMA_REGISTRY)darwin-amd64 $(BUILD_ARGS) $(CMD_REGISTRY)
	@CGO_ENABLED=0 GOOS=darwin go build -o $(BIN_PREFIX_SCHEMA_REGISTRY)darwin-arm64 $(BUILD_ARGS) $(CMD_REGISTRY)

	@CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN_PREFIX_SCHEMA_CLI)darwin-amd64 $(BUILD_ARGS) $(CMD_CLI)
	@CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o $(BIN_PREFIX_SCHEMA_CLI)darwin-arm64 $(BUILD_ARGS) $(CMD_CLI)
